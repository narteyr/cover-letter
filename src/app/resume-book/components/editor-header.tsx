...
import { HomeLine, Download01, Edit01, File05, Menu02, MessageChatCircle, ChevronDown, Target01 } from "@untitledui/icons";
        ...
        <div className="shrink-0">
          <Dropdown.Root>
            ...
            <Dropdown.Popover className="w-56">
              <Dropdown.Menu selectionMode="none" disallowEmptySelection={false}>
                <Dropdown.Section>
                  <Dropdown.SectionHeader>{jobs.length > 0 ? "Change job" : "Job"}</Dropdown.SectionHeader>
                  {jobs.length > 0 ? (
                    jobs.map((job) => (
                      <Dropdown.Item key={job.id} id={`job-${job.id}`} onAction={() => onJobSelect(job.id)}>
                        {job.title} at {job.company}
                      </Dropdown.Item>
                    ))
                  ) : onAddJob ? (
                    <Dropdown.Item id="add-job" icon={Target01} onAction={onAddJob}>
                      Add job details
                    </Dropdown.Item>
                  ) : null}
                </Dropdown.Section>
